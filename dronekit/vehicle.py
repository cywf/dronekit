...
class Vehicle:
    ...
    def connect(self, wait_ready: bool = False, timeout: float = 30.0):
        ...
        try:
            ...
            while time.time() - start_time < timeout:
                msg = self._master.recv_match(type='HEARTBEAT', blocking=True, timeout=1.0)
                if msg:
    ...
    def _process_heartbeat(self, msg):
        ...
        with self._lock:
            ...
            self._mode = VehicleMode(mode_mapping.get(custom_mode, f"MODE_{custom_mode}"))
    ...
    def _process_sys_status(self, msg):
        ...
        with self._lock:
            ...
            self._battery.current = msg.current_battery / 100.0 if msg.current_battery != -1 else 0
            self._battery.level = msg.battery_remaining if msg.battery_remaining != -1 else 0
...
def connect(connection_string: str, wait_ready: bool = False, timeout: float = 30.0) -> Vehicle:
    """
